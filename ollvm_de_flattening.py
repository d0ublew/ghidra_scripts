...
from ghidra.program.model.pcode import PcodeOp, RawPcode, RawPcodeImpl, Varnode
...
def find_const_def_blocks(mem, state_var_size, pcode, depth, res, def_block):
    if depth > 3:
        ...
        logging.debug(
            "finding state var def in pcode %s of block %s, depth %d"
            % (pcode, pcode.getParent(), depth)
        )
        if pcode.getOpcode() == PcodeOp.COPY:
            ...
            if input_var.isConstant():
                logging.debug(
                    "%s defines state var to const: %s" % (def_block, input_var)
                )
                if def_block not in res:
                    res[def_block] = input_var.getOffset()
                else:
                    logging.warning(
                        "%s already defines state var to const %s, skipped"
                        % (def_block, res[def_block])
                    )
            else:
                # if input var is in ram, read its value
                if input_var.getAddress().getAddressSpace().getName() == "ram":
                    if input_var.isAddress():
                        if state_var_size == 4:
                            ...
                            logging.warning(
                                "state var size %d not supported" % state_var_size
                            )
                    else:
                        logging.warning(
                            "def of non-const input_var %s not found" % input_var
                        )
                # not ram or const, trace back to const def
                else:
                    find_const_def_blocks(
                        mem,
                        state_var_size,
                        input_var.getDef(),
                        depth + 1,
            ...
            for input_var in pcode.getInputs().tolist():
                find_const_def_blocks(
                    mem, state_var_size, input_var.getDef(), depth + 1, res, def_block
                )
        else:
...
class Patcher(object):
    ...
    def patch_conditional_jump(self, ins, true_block, false_block, target_block):
        ...
        for index in range(pcode_len):
            ...
            if pcode_len >= 4 and index < pcode_len - 3:
                if (
                    p_code[index].opcode == PcodeOp.COPY
                    and p_code[index + 1].opcode == PcodeOp.BRANCH
                    and p_code[index + 2].opcode == PcodeOp.COPY
                    and p_code[index + 3].opcode == PcodeOp.INT_ZEXT
                ):
                    reg_data_idx = index
                # ppc64le
                if (
                    p_code[index].opcode == PcodeOp.COPY
                    and p_code[index + 1].opcode == PcodeOp.INT_SUB
                    and p_code[index + 2].opcode == PcodeOp.INT_RIGHT
                    and p_code[index + 3].opcode == PcodeOp.INT_AND
                ):
        ...
        if arch == "MIPS":
            if (
                p_code[0].opcode == PcodeOp.INT_NOTEQUAL
                and p_code[1].opcode == PcodeOp.CBRANCH
            ):
            ...
            while target_start_addr <= target_end_addr:
                ...
                if (
                    current_pcode.opcode == PcodeOp.INT_XOR
                    and current_pcode.getInput(1).isConstant() is True
                ):
                    value_inputs = array(Varnode, [current_pcode.getInput(1)])
                    # (register,0x10,8) v0
                    value_output = Varnode(
                        addrs.getAddress(addrs.registerSpace.spaceID, 0x10), 8
                    )
            ...
            if inst_before_pcode.getInput(0).offset == 0x18 and value is not None:
                ...
                value_inputs = array(
                    Varnode,
                    [Varnode(addrs.getAddress(addrs.constantSpace.spaceID, 0), 8)],
                )
                value_output = Varnode(
                    addrs.getAddress(addrs.registerSpace.spaceID, 0x10), 8
                )
        ...
        for idx in range(reg_data_idx):
            ...
            if arch == "MIPS" and reg_opcode == PcodeOp.INT_NOTEQUAL:
                reg_inputs = array(
                    Varnode,
                    [
                        Varnode(addrs.getAddress(addrs.registerSpace.spaceID, 0x8), 8),
    ...
    def do_patch(self, link):
        ...
        if len(link) == 2:
            ...
            logging.debug(
                "patching unconditional jump at %s to %s" % (patch_addr, target_addr)
            )
...
def find_state_var_defs(mem, state_var):
    ...
    for state_var_def in phi_node.getInputs().tolist():
        ...
        logging.debug(
            "output %s of pcode %s in block %s defines state var"
            % (state_var_def, pcode, pcode.getParent())
        )
...
def gen_cfg(const_map, state_var_defs):
    ...
    for def_block, const in state_var_defs.items():
        ...
        # conditional jump
            ...
            logging.debug(
                "%s true out: %s, false out %s" % (def_block, true_out, false_out)
            )
            ...
            if true_out in state_var_defs:
                ...
                if true_out_const not in const_map:
                    logging.warning(
                        "true out cannot find map from const 0x%x to block"
                        % true_out_const
                    )
                    continue
                ...
                if false_out in state_var_defs:
                    ...
                    if false_out_const not in const_map:
                        logging.warning(
                            "false out cannot find map from const 0x%x to block"
                            % false_out_const
                        )
                        continue
                    else:
                ...
                # false out doesn't have const def, then use the def in current block for the false out
                    ...
                    logging.warning(
                        "mapping of const %s in block %s not found" % (const, def_block)
                    )
                    continue
            ...
            # false out block has state var def
                ...
                logging.warning(
                    "no state var def in either trueout or falseout of block %s"
                    % def_block
                )
        else:
            logging.warning(
                "output block counts %d not supported" % def_block.getOutSize()
            )
